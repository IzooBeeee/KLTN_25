<template>
  <div class="min-h-screen bg-[#f0f4f8] font-['Inter'] p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      ...
      <div
        class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-5 shadow-sm text-white flex items-center justify-between">
        <div>
      ...
      <div class="bg-white rounded-2xl p-5 shadow-sm border border-red-100 flex items-center justify-between">
        <div>
          ...
          <div class="text-3xl font-black text-red-500">{{danhSachGoiTin.filter(x => x.trang_thai ===
            'inactive').length}}</div>
        </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100">
              <th class="text-left text-xs font-bold text-gray-400 uppercase tracking-widest px-5 py-4">Thông tin gói
              </th>
              <th class="text-right text-xs font-bold text-gray-400 uppercase tracking-widest px-5 py-4">Giá tiền</th>
              <th class="text-center text-xs font-bold text-gray-400 uppercase tracking-widest px-5 py-4">Số lượng tin
              </th>
            ...
            <tr v-for="(value, index) in paginatedList" :key="index" class="hover:bg-gray-50 transition-colors">
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  <div
                    class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-violet-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                  ...
                  <div>
                    ...
                    <span class="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">#PK{{
                      String(value.id).padStart(3, '0') }}</span>
                  </div>
                </div>
              </td>
              ...
              <td class="px-5 py-4 text-center">
                <span
                  class="inline-flex items-center gap-1 px-3 py-1 bg-blue-50 text-blue-600 text-xs font-bold rounded-full">
              ...
              <td class="px-5 py-4 text-center">
                <span @click="changeStatus(value, value.trang_thai !== 'active')"
                  class="cursor-pointer inline-flex items-center px-3 py-1 rounded-full text-xs font-bold transition"
              ...
              <td class="px-5 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button @click="id_can_xoa = value.id; showDeleteModal = true"
                    class="w-9 h-9 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white hover:shadow-lg hover:shadow-red-100 transition-all active:scale-90"
                    title="Xóa">
                    <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="px-5 py-3 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div v-if="totalPages > 1" class="flex items-center gap-1">
          ...
          <button v-for="page in visiblePages" :key="page" @click="goToPage(page)"
            ...
            :class="page === currentPage ? 'bg-blue-600 text-white shadow-sm' : 'border border-gray-200 text-gray-500 hover:bg-gray-50'">{{
              page }}</button>
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
          <div class="flex items-center justify-between p-6 border-b border-gray-100">
            ...
            <button @click="showCreateModal = false"
              class="w-8 h-8 rounded-lg bg-gray-100 text-gray-400 hover:text-gray-600 flex items-center justify-center transition">
          ...
          <div class="p-6 space-y-4">
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Tên gói tin <span
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Giá tiền
                  (VNĐ)</label>
                <input v-model="goi_tin_create.gia" type="number" placeholder="0"
                  class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" />
              </div>
              <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Số lượng
                  tin</label>
            ...
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Thời hạn
                (Ngày)</label>
          ...
          <div class="flex gap-3 p-6 pt-0">
            <button @click="showCreateModal = false"
              class="flex-1 py-3 border-2 border-gray-200 text-gray-500 font-bold rounded-xl hover:bg-gray-50 transition text-sm">Hủy
              bỏ</button>
            <button @click="themMoiGoiTin(); showCreateModal = false"
              class="flex-[2] py-3 bg-gradient-to-r from-blue-600 to-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-blue-100 hover:-translate-y-1 hover:shadow-xl transition-all active:scale-95 text-sm">Lưu
              gói tin</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="showUpdateModal"
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
          ...
          <div class="flex gap-3 p-6 pt-0">
            ...
            <button @click="capNhatGoiTin(); showUpdateModal = false"
              class="flex-1 py-3 bg-gradient-to-r from-amber-500 to-amber-600 text-white font-bold rounded-xl shadow-sm hover:-translate-y-0.5 transition-all text-sm">Lưu
              thay đổi</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="showDeleteModal"
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden text-center">
          <div class="p-8">
            ...
            <p class="text-sm text-gray-400 mb-6">Hành động này không thể hoàn tác. Bạn có chắc chắn muốn xóa gói tin
              này?</p>
            <div class="flex gap-3">
              <button @click="showDeleteModal = false"
                class="flex-1 py-3 border-2 border-gray-200 text-gray-500 font-bold rounded-xl hover:bg-gray-50 transition text-sm">Hủy</button>
              <button @click="xoaGoiTin(); showDeleteModal = false"
                class="flex-1 py-3 bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded-xl shadow-sm hover:-translate-y-0.5 transition-all text-sm">Xóa
                ngay</button>
            </div>
          </div>
        </div>
      ...
      axios
        ...
        .then((response) => {
          if (
            ...
            this.$toast.error(
              `<span class="text-nowrap">Thất Bại <b>${response.data.message || "Có lỗi xảy ra"
              }</b></span>`
            );
          }
        })
        .catch((error) => {
          ...
          if (
            ...
            this.$toast.error(
              `<span class="text-nowrap">Thất Bại <b>${error.response.data.message || "Có lỗi xảy ra"
              }</b></span>`
            );
          }
        });
