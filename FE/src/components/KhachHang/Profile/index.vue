<template>
  <div class="min-h-screen bg-[#f0f4f8] font-['Inter']">
    <!-- Hero Banner -->
    <div class="relative h-56 bg-gradient-to-r from-[#1e3a8a] via-[#1d4ed8] to-[#0ea5e9] overflow-hidden">
      <div class="absolute inset-0 opacity-20" style="
          background-image: url('https://images.unsplash.com/photo-1549490349-8643362247b5?w=1600&q=60');
          background-size: cover;
          background-position: center;
        "></div>
    </div>

    <div class="container mx-auto max-w-6xl px-4 -mt-10 pb-12 relative z-10">
      <!-- Profile Header Card -->
      <div
        class="bg-white rounded-3xl shadow-xl p-6 pt-16 sm:pt-6 mb-6 flex flex-col sm:flex-row items-center gap-5 relative z-20">
        <div class="relative -mt-12 sm:-mt-14 flex-shrink-0">
          <img :src="avatarUrl"
            class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border-4 border-white shadow-lg object-cover" alt="Avatar" />
          <span
            class="absolute -bottom-2 -right-2 w-7 h-7 bg-green-500 border-2 border-white rounded-full flex items-center justify-content-center">
        ...
        <div class="flex-1 text-center sm:text-left">
          <div class="flex items-center justify-center sm:justify-start gap-2 mb-1">
            <h1 class="text-2xl font-black text-gray-900">
              {{ profile.ten || "Khách hàng" }}
            </h1>
            <span
              class="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">
          ...
          <p class="text-start text-gray-400">Thành viên từ {{ joinDate }}</p>
        </div>
        <button @click="updateProfile" :disabled="saving" style="border-radius: 9999px;"
          ...
          <span v-if="saving" class="w-4 h-4 animate-spin"></span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Xác nhận mật
                    khẩu</label>
                  <input v-model="password.re_password" type="password" placeholder="••••••••"
                    class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition" />
                </div>
              </div>
              <button @click="changePassword" :disabled="changingPwd"
                ...
                <span v-if="changingPwd"
                  class="w-4 h-4 border-2 border-amber-400/30 border-t-amber-500 rounded-full animate-spin"></span>
        ...
        <div class="space-y-6">
          ...
          <!-- Upgrade Banner -->
          <div class="relative rounded-2xl overflow-hidden p-6 text-white"
            ...
            <p class="text-sm text-blue-200 mb-4 leading-relaxed">
              Đăng BĐS, tiếp cận hàng nghìn khách hàng tiềm năng.
            </p>
            <router-link to="/khach-hang/nang-cap-moi-gioi"
              class="inline-flex items-center gap-2 px-5 py-2.5 bg-white  text-dark text-sm font-black rounded-xl hover:bg-blue-50 transition shadow-lg">
              Nâng cấp ngay
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="bg-white rounded-2xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <router-link to="/khach-hang/yeu-thich" class="text-xs font-bold text-blue-600 hover:underline">Xem tất
                cả</router-link>
            </div>
            ...
            <div v-for="f in favorites.slice(0, 3)" :key="f.id" class="flex gap-3 mb-3 last:mb-0">
              <img :src="f.bat_dong_san?.anh_dai_dien_url ||
                'https://placehold.co/54x54?text=No'
                " class="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
              <div class="overflow-hidden">
                <p class="text-sm font-semibold text-gray-800 truncate">
                  {{ f.bat_dong_san?.tieu_de }}
                </p>
                <p class="text-xs text-gray-400">
                  {{ f.bat_dong_san?.dia_chi?.quan?.ten_quan }}
                </p>
                <p class="text-sm font-bold text-green-600">
                  {{ formatCurrency(f.bat_dong_san?.gia) }}
                </p>
              </div>
            </div>
          </div>
...
const avatarUrl = computed(
  () =>
    `https://ui-avatars.com/api/?name=${encodeURIComponent(
      profile.ten || "KH"
    )}&background=1e3a8a&color=fff&size=80`
);
