...
def normalize_actor(payload: Dict[str, Any]) -> str:
    raw_actor = str(payload.get("actor") or "").lower()
    raw_role = str(payload.get("role") or payload.get("user_type") or "").lower()

    broker_values = ["broker", "moi-gioi", "moi_gioi", "agent"]
    customer_values = ["customer", "khach-hang", "khach_hang", "user"]

    if raw_actor in broker_values:
        return "broker"

    if raw_actor in customer_values:
        return "customer"

    if raw_role in broker_values:
        return "broker"

    if raw_role in customer_values:
        return "customer"
...
class RolePolicy:
    ...
    BROKER_ONLY: set = {
        ...
        "broker_appointments",
        "broker_pending_appointments",
        "broker_confirmed_appointments",
        "broker_leads",
        "broker_profile_help",
        "broker_help",
        "general_broker_help",
...
class RuleBasedNLU:
    ...
    def analyze(self, message: str, memory: ChatMemory) -> Dict[str, Any]:
        ...
        if memory.actor == "broker":
            package_status_keywords = [
                "tôi còn bao nhiêu lượt đăng", "toi con bao nhieu luot dang",
                "còn bao nhiêu lượt đăng", "con bao nhieu luot dang",
                "số lượt đăng còn lại", "so luot dang con lai",
                "gói của tôi còn hạn không", "goi cua toi con han khong",
                "gói của tôi hết hạn chưa", "goi cua toi het han chua",
                "kiểm tra gói hiện tại", "kiem tra goi hien tai",
                "trạng thái gói tin", "trang thai goi tin",
            ]
            if any(k in t or k in t_ascii for k in package_status_keywords):
                return {
                    "intent": "broker_package_status",
                    "context": "broker",
                    "entities": {},
                    "confidence": 1.0,
                    "raw": t,
            ...
            pending_app_keywords = [
                "lịch chờ xác nhận", "lich cho xac nhan",
                "lịch đang chờ", "lich dang cho",
                "lịch khách chờ xác nhận", "lich khach cho xac nhan",
                "khách chờ xác nhận", "khach cho xac nhan",
                "lịch chưa xác nhận", "lich chua xac nhan",
            ]
            if any(k in t or k in t_ascii for k in pending_app_keywords):
                return {
                    "intent": "broker_pending_appointments",
                    "context": "broker",
                    "entities": {},
                    "confidence": 1.0,
                    "raw": t,
            ...
            confirmed_app_keywords = [
                "lịch đã xác nhận", "lich da xac nhan",
                "lịch đã duyệt", "lich da duyet",
            ]
            if any(k in t or k in t_ascii for k in confirmed_app_keywords):
                return {
                    "intent": "broker_confirmed_appointments",
                    "context": "broker",
                    "entities": {},
                    "confidence": 1.0,
                    "raw": t,
...
class ResponseGenerator:
    ...
    def simple(self, intent: str) -> Tuple[str, List[str]]:
        mapping = {
            ...
            "broker_package_status": (
                "Mình chưa đọc được trạng thái gói của bạn từ chatbot. Bạn có thể kiểm tra trong mục Gói tin/Tài khoản môi giới để xem số lượt đăng còn lại và ngày hết hạn.",
                ["Gói tin đăng BĐS giá bao nhiêu?", "Cách mua gói tin", "Hướng dẫn đăng bài"],
            ),
            "broker_pending_appointments": (
                "Bạn có thể vào mục Lịch hẹn xem nhà trên dashboard môi giới, chọn tab Lịch chờ xác nhận để xem các yêu cầu mới từ khách hàng.",
                ["Lịch khách hẹn xem nhà", "Gói tin đăng BĐS giá bao nhiêu?", "Hướng dẫn đăng bài"],
            ),
            "broker_confirmed_appointments": (
                "Bạn có thể vào mục Lịch hẹn xem nhà trên dashboard môi giới, chọn tab Đã xác nhận để xem các lịch đã được xác nhận.",
                ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            ),
        }
        return mapping.get(
            intent,
...
# ===== BROKER APPOINTMENTS HELPERS =====

def get_auth_token(payload: Dict[str, Any]) -> Optional[str]:
    """Extract and normalize auth token from payload."""
    token = (
        payload.get("token")
...
def format_broker_appointment_api_items(items: List[Dict[str, Any]], title: str) -> str:
    """Format appointment items from Laravel API response."""
    lines = [title]

    for i, item in enumerate(items[:5], start=1):
        bds = item.get("bat_dong_san") or {}
        kh = item.get("khach_hang") or {}

        customer = kh.get("ten") or "Không rõ"
        phone = kh.get("so_dien_thoai") or "Chưa có"
        email = kh.get("email") or "Chưa có"

        prop = bds.get("tieu_de") or "BĐS không rõ"
        address = bds.get("dia_chi") or "Chưa có địa chỉ"

        date = item.get("ngay_hen") or ""
        time = item.get("gio_hen") or ""
        status = item.get("status_label") or item.get("trang_thai") or "Không xác định"
        note = item.get("ghi_chu") or ""
...
class BDSChatbot:
    ...
    def handle_broker_package_status(self, mem: ChatMemory, payload: Dict[str, Any], start: float) -> Dict[str, Any]:
        session_id = (payload.get("session_id") or "guest_default")[:100]
        user = payload.get("user") or {}
        broker_id = payload.get("broker_id") or payload.get("user_id") or user.get("id")
        ...
        if broker_id:
            try:
                ...
                cur = conn.cursor(dictionary=True)
                cur.execute("SELECT so_tin_con_lai, ngay_het_han_goi FROM tai_khoans WHERE id = %s", (broker_id,))
                row = cur.fetchone()
                cur.close()
                conn.close()
                if row and row.get("so_tin_con_lai") is not None:
                    rem = row.get("so_tin_con_lai")
                    exp = row.get("ngay_het_han_goi") or "chưa xác định"
                    if isinstance(exp, datetime):
                        exp = exp.strftime("%d/%m/%Y")
                    resp = f"Tài khoản của bạn hiện còn <b>{rem} lượt đăng tin</b>. Ngày hết hạn gói: {exp}."
            except Exception as e:
                logger.warning("pkg_status_db_fail | %s", e)
                
        if not resp:
            resp = "Mình chưa đọc được trạng thái gói của bạn từ chatbot. Bạn có thể kiểm tra trong mục Gói tin/Tài khoản môi giới để xem số lượt đăng còn lại và ngày hết hạn."
            
        quick = ["Gói tin đăng BĐS giá bao nhiêu?", "Cách mua gói tin", "Hướng dẫn đăng bài"]
        an = {"intent": "broker_package_status", "context": "broker", "entities": {}}
        self.memory.update(session_id, payload.get("message", ""), resp, an, [])
        return self.wrap(resp, an, [], quick, start)
    ...
    def broker_appointment_query_error(self, start: float) -> Dict[str, Any]:
        return self.wrap(
            "Mình chưa truy vấn được lịch hẹn lúc này. Bạn thử lại sau hoặc xem trong dashboard môi giới.",
            {"intent": "broker_appointments", "context": "broker", "error": "query_failed"},
            [],
            ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            start
        )

    def broker_no_id_response(self, start: float) -> Dict[str, Any]:
        return self.wrap(
            "Mình chưa xác định được tài khoản môi giới hiện tại. Bạn vui lòng đăng nhập lại hoặc xem trong mục Lịch hẹn xem nhà trên dashboard.",
            {"intent": "broker_appointments", "context": "broker", "error": "missing_broker_id"},
            [],
            ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            start
        )

    def broker_no_appointments_response(self, status: str = None, start: float = None) -> Dict[str, Any]:
        if status == "pending":
            text = "Hiện bạn chưa có lịch hẹn nào đang chờ xác nhận."
        elif status == "confirmed":
            text = "Hiện bạn chưa có lịch hẹn nào đã xác nhận."
        else:
            text = "Hiện bạn chưa có lịch hẹn xem nhà nào từ khách hàng."

        return self.wrap(
            text,
            {"intent": "broker_appointments", "context": "broker", "status": status},
            [],
            ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            start
        )

    def broker_missing_token_response(self, start: float) -> Dict[str, Any]:
        return self.wrap(
            "Mình chưa xác định được tài khoản môi giới hiện tại. Bạn vui lòng đăng nhập lại hoặc xem trong mục Lịch hẹn xem nhà trên dashboard.",
            {"intent": "broker_appointments", "context": "broker", "error": "missing_token"},
            [],
            ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            start
        )

    def broker_unauthorized_response(self, start: float) -> Dict[str, Any]:
        return self.wrap(
            "Phiên đăng nhập môi giới có thể đã hết hạn. Bạn vui lòng đăng nhập lại để mình xem được lịch hẹn.",
            {"intent": "broker_appointments", "context": "broker", "error": "unauthorized"},
            [],
            ["Lịch chờ xác nhận", "Lịch đã xác nhận", "Gói tin đăng BĐS giá bao nhiêu?"],
            start
        )

    def handle_broker_appointments(self, mem: ChatMemory, payload: Dict[str, Any], start: float) -> Dict[str, Any]:
        session_id = (payload.get("session_id") or "guest_default")[:100]
        try:
            ...
            return self.wrap(text, an, [], quick, start)
        except Exception as e:
            logger.exception("fetch_broker_appointments_fail")
            return self.broker_appointment_query_error(start)

    def handle_broker_pending_appointments(self, mem: ChatMemory, payload: Dict[str, Any], start: float) -> Dict[str, Any]:
        session_id = (payload.get("session_id") or "guest_default")[:100]
        try:
    ...
    def handle_broker_confirmed_appointments(self, mem: ChatMemory, payload: Dict[str, Any], start: float) -> Dict[str, Any]:
        session_id = (payload.get("session_id") or "guest_default")[:100]
        try:
    ...
    def process_broker(self, message: str, payload: Dict[str, Any], mem: "ChatMemory", start: float) -> Dict[str, Any]:
        msg_ascii = normalize_ascii(message)
        session_id = payload.get("session_id") or "guest_default"

        # 1. Lịch hẹn (Appointments)
        if any(k in msg_ascii for k in [
            "dat lich xem nha", "lich xem nha", "xem lich hen",
            "lich hen", "lich khach", "lich cua toi", "lich khen",
            "lịch khách hẹn xem nhà", "lich khach hen xem nha",
            "lịch khách đặt xem nhà", "lich khach dat xem nha",
        ]):
            return self.handle_broker_appointments(mem, payload, start)
        ...
        if any(k in msg_ascii for k in [
            ...
            return self.handle_broker_pending_appointments(mem, payload, start)
            
        if any(k in msg_ascii for k in [
            "lịch đã xác nhận", "lich da xac nhan",
            "lịch đã duyệt", "lich da duyet",
        ]):
            return self.handle_broker_confirmed_appointments(mem, payload, start)
            
        # 2. Gói tin / Trạng thái (Package/Posting/Status)
        if any(k in msg_ascii for k in [
            "tôi còn bao nhiêu lượt đăng", "toi con bao nhieu luot dang",
            "còn bao nhiêu lượt đăng", "con bao nhieu luot dang",
            "số lượt đăng còn lại", "so luot dang con lai",
            ...
            return self.handle_broker_package_status(mem, payload, start)
            
        rule = self.nlu.analyze(message, mem)
        intent = rule.get("intent")
        
        if intent == "package_info" or any(k in msg_ascii for k in ["gói tin", "goi tin", "bảng giá gói tin", "bang gia goi tin", "xem lại gói tin", "gói tin đăng bđs giá bao nhiêu", "goi tin dang bds gia bao nhieu"]):
            try:
                package_payload = deepcopy(payload)
                routed = self.router.handle_package_action("goi_tin/list_packages", rule, package_payload)
                routed.setdefault("intent", "package_info")
                routed.setdefault("context", "broker")
                routed.setdefault("processing_time", round(time.time() - start, 3))
                return routed
            except Exception:
                logger.exception("broker package fetch failed")
                # Return standard static package prices if DB fails
                draft = (
                    "Dưới đây là bảng giá các gói tin đăng dành cho môi giới:\n"
                    "• Gói Cơ Bản: 50.000đ/7 ngày, 3 tin\n"
                    "• Gói Tiêu Chuẩn: 100.000đ/15 ngày, 10 tin\n"
                    "• Gói VIP: 250.000đ/30 ngày, 30 tin\n"
                    "• Gói Cao Cấp: 500.000đ/60 ngày, 100 tin"
                )
                quick = ["Cách mua gói tin", "Hướng dẫn đăng bài"]
                rule["intent"] = "package_info"
                self.memory.update(session_id, message, draft, rule, [])
                return self.wrap(draft, rule, [], quick, start)
            
        if intent == "package_buy_guide" or any(k in msg_ascii for k in ["cách mua gói tin", "mua gói tin", "thanh toán gói"]):
            draft, quick = self.responses.simple("package_buy_guide")
            rule["intent"] = "package_buy_guide"
            self.memory.update(session_id, message, draft, rule, [])
            return self.wrap(draft, rule, [], quick, start)
            
        if intent == "posting_guide" or any(k in msg_ascii for k in ["hướng dẫn đăng bài", "hướng dẫn đăng tin", "cách đăng bài", "cách đăng tin"]):
            draft, quick = self.responses.simple("posting_guide")
            rule["intent"] = "posting_guide"
            self.memory.update(session_id, message, draft, rule, [])
            return self.wrap(draft, rule, [], quick, start)

        # 3. Help & Chặn Customer
        if intent == "appointment":
            draft = "Ở chế độ môi giới, bạn có thể xem lịch khách hẹn xem nhà trong mục Lịch khách hẹn xem nhà."
            quick = ["Lịch khách hẹn xem nhà", "Lịch chờ xác nhận", "Lịch đã xác nhận"]
            rule["intent"] = "broker_feature_redirect"
            self.memory.update(session_id, message, draft, rule, [])
            return self.wrap(draft, rule, [], quick, start)
            
        if intent in ["search_property", "valuation", "property_detail", "refine_search"] or any(k in msg_ascii for k in ["tìm bđs", "tim bds", "định giá", "dinh gia", "gần biển", "gan bien", "đặt lịch xem nhà", "dat lich xem nha"]):
            draft = "Chế độ này dành cho môi giới nên mình ưu tiên hỗ trợ gói tin, đăng bài, quản lý tin và lịch khách hẹn. Nếu bạn muốn tìm BĐS hoặc định giá như khách hàng, hãy chuyển sang khu khách hàng."
            quick = ["Gói tin đăng BĐS giá bao nhiêu?", "Hướng dẫn đăng bài", "Lịch khách hẹn xem nhà"]
            rule["intent"] = "broker_feature_redirect"
            self.memory.update(session_id, message, draft, rule, [])
            return self.wrap(draft, rule, [], quick, start)
            
        # 4. Fallback Catch-all cho Broker
        ...
        quick = ["Gói tin đăng BĐS giá bao nhiêu?", "Cách mua gói tin", "Hướng dẫn đăng bài", "Lịch khách hẹn xem nhà", "Tôi còn bao nhiêu lượt đăng?"]
        rule["intent"] = "broker_help"
        self.memory.update(session_id, message, draft, rule, [])
        return self.wrap(draft, rule, [], quick, start)

    def handle_broker_appointment_router(self, message: str, payload: Dict[str, Any], mem: "ChatMemory", start: float) -> Dict[str, Any]:
        t = normalize_ascii(message)

        if "cho xac nhan" in t or "dang cho" in t or "chua xac nhan" in t:
            return self.handle_broker_pending_appointments(mem, payload, start)

        if "da xac nhan" in t or "da duyet" in t:
            return self.handle_broker_confirmed_appointments(mem, payload, start)
    ...
    def process(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        session_id_raw = str(payload.get("session_id") or "guest")[:100]
        if session_id_raw.startswith(f"{actor}:"):
            session_id = session_id_raw
        else:
            user_ident = payload.get("user_id") or payload.get("broker_id") or session_id_raw
            # clean wrong prefix if present
            if str(user_ident).startswith("broker:") or str(user_ident).startswith("customer:"):
                user_ident = str(user_ident).split(":", 1)[1]
            session_id = f"{actor}:{user_ident}"
        ...
        logger.info(
            "CHAT_PAYLOAD actor=%s role=%s session_id=%s message=%s",
            payload.get("actor"),
            payload.get("role"),
        ...
        logger.info("NORMALIZED actor=%s message_ascii=%s", actor, message_ascii)

        broker_appointment_keywords = [
            "lich khach hen xem nha",
            "lich hen xem nha",
            "khach hen xem nha",
            "lich khach dat xem nha",
            "lich cho xac nhan",
            "lich da xac nhan",
            "lich dang cho",
            "lich chua xac nhan",
        ...
        if actor == "broker" and any(k in message_ascii for k in broker_appointment_keywords):
            logger.info("BROKER_APPOINTMENT_HARD_GUARD_TRIGGERED message=%s", message)
            try:
                return self.handle_broker_appointment_router(message, payload, mem, start)
            except Exception:
                logger.exception("broker appointment query failed")
                return self.broker_appointment_query_error(start)

        if actor == "broker":
            return self.process_broker(message, payload, mem, start)
...
def chat():
    ...
    try:
        ...
        raw_actor = str(payload.get("actor") or "").lower()
        if raw_actor in ["broker", "moi-gioi", "moi_gioi", "agent"]:
            return jsonify({
                "success": True,
                "response": "Mình chưa xử lý được yêu cầu lúc này. Bạn thử lại sau hoặc xem trên dashboard môi giới nhé.",
                "intent": "broker_help",
                "context": "broker",
                "suggestions": [],
                "quick_replies": [
